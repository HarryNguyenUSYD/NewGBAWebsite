...
const PartnerListing = ({ props } : { props: PartnerListingProps }) => {
    const languageContext = useLanguage();
    ...
    return (
        ...
        >
            ...
            <div className="w-full flex flex-col justify-end items-start gap-3">
                ...
                >
                    <FiFileText />
                    {languageContext?.language == "en" ? "View Partner Description" : "Xem mô tả công việc"}
                </Link>
                <Link
                    download={true}
                    href={"/"}
                ...
                >
                    <HiOutlineDownload />
                    {languageContext?.language == "en" ? "Download Partner Description" : "Tải mô tả công việc"}
                </Link>
            </div>
        </div>
    )
    ...
    return (
        <div className="w-full h-auto px-10 my-10 flex flex-col justify-start items-center gap-10">
            {Array.from({ length: 10 }, (_, j) => j + 1).map((_, k) => (
                <PartnerListing key={`position_${k}`} props={{
